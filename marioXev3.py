...
MAX = 3.5 * 360
MIN = 0
STEPS = itertools.cycle(range(6))

EV3_SETTINGS = {"protocol":ev3.BLUETOOTH, "host":"00:16:53:81:D7:E2"}
with (ev3.EV3(**EV3_SETTINGS) as brick, 
    ev3.TwoWheelVehicle(0.04747, 0.1224, ev3_obj=brick) as car,
    ev3.Motor(ev3.PORT_C, ev3_obj=brick) as small_motor,
    ev3.Gyro(ev3.PORT_1, ev3_obj=brick) as gyro):
    small_motor.move_by(360, speed=80).start(thread=False)
    speed = 0
    turn = 0
    ...
    if __name__ == "__main__":
        # Initialize Marios
        mario = Mario(do_log = True, 
            accelerometer_hooks = my_accelerometer_hook, default_volume=0,
            tile_event_hooks = my_tile_hook, pants_event_hooks=None)
